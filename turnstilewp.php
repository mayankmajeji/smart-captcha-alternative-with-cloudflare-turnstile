...
// Plugin version
define('TURNSTILEWP_VERSION', '1.0.0');
define('TURNSTILEWP_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('TURNSTILEWP_PLUGIN_URL', plugin_dir_url(__FILE__));
define('TURNSTILEWP_PLUGIN_BASENAME', plugin_basename(__FILE__));

// Autoloader
require_once TURNSTILEWP_PLUGIN_DIR . 'includes/class-loader.php';

// Initialize the plugin
function turnstilewp_init()
{
	// Load text domain
	...
	load_plugin_textdomain('smart-cloudflare-turnstile', false, dirname(TURNSTILEWP_PLUGIN_BASENAME) . '/i18n/languages');

	// Initialize main plugin class as singleton
	\TurnstileWP\Init::get_instance()->init();
}

add_action('plugins_loaded', 'turnstilewp_init');

// Activation hook
register_activation_hook(__FILE__, function () {
	// Activation tasks will be handled by Init class
	require_once TURNSTILEWP_PLUGIN_DIR . 'includes/class-init.php';
	\TurnstileWP\Init::get_instance()->activate();
});
...
register_deactivation_hook(__FILE__, function () {
	...
	require_once TURNSTILEWP_PLUGIN_DIR . 'includes/class-init.php';
	\TurnstileWP\Init::get_instance()->deactivate();
});
